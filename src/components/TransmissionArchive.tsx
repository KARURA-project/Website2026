      ...
      {items.map((it) => (
        <div key={it.id} className="py-6">
          <Link href={it.link || '#'} className="block">
            <div className="font-display text-lg font-bold mb-1">{it.title}</div>
            <div className="text-sm text-[#0A0A0A]/50 mb-2">{new Date(it.date).toLocaleDateString()}</div>
            <p className="text-sm text-[#0A0A0A]/65">{it.description}</p>
          </Link>
        </div>
      ))}
    </div>
          ...
          {items.map((it) => (
            <article key={it.id} className="border border-[#0A0A0A]/8 p-6 rounded-lg bg-white">
              ...
              <div className="flex items-center justify-between">
                <div className="font-mono text-xs text-[#0A0A0A]/40">{it.date}</div>
